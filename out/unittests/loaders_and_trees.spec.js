...
g.test('end2end').fn(async t => {
  ...
  await exp(2, new TestQuerySingleCase('suite2', ['foof'], ['bluh', 'a'], {}), 'fail', logs => logs.length === 1 && logs[0].startsWith('"FAIL: Error: bye\\n') && logs[0].indexOf('loaders_and_trees.spec.') !== -1);
});
...
g.test('iterateCollapsed').fn(async t => {
  ...
  await testIterateCollapsed(t, ['suite1:bar,buzz,buzz:*'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:*']); // Some effect
