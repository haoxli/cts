...
// DOM generation
function makeTreeNodeHTML(tree) {
  if ('children' in tree) {
...
function makeTreeNodeHeaderHTML(query, description, runSubtree, isLeaf) {
  const div = $('<div>').addClass('nodeheader');
  const href = `?${worker ? 'worker&' : ''}${debug ? 'debug&' : ''}q=${query.toString()}`;
  $('<button>').addClass(isLeaf ? 'leafrun' : 'subtreerun').attr('alt', 'Run subtree').attr('title', 'Run subtree').on('click', async () => {
    await runSubtree();
    updateJSON();
  }).appendTo(div);
